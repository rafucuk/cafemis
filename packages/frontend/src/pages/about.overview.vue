	...
	<FormSection>
		<div class="_gaps_m">
			<MkKeyValue :copy="version">
				<template #key>Cafemis</template>
				<template #value>{{ version }}</template>
			</MkKeyValue>
			<div v-html="i18n.tsx.poweredByMisskeyDescription({ name: instance.name ?? host })">
			</div>
			<FormLink to="/about-cafemis">
				<template #icon><i class="ti ti-info-circle"></i></template>
				{{ i18n.ts.aboutMisskey }}
			</FormLink>
			<FormLink v-if="instance.repositoryUrl || instance.providesTarball" :to="instance.repositoryUrl || `/tarball/cafemis-${version}.tar.gz`" external>
