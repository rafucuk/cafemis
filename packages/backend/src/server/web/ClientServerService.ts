...
export class ClientServerService {
	...
	private async manifestHandler(reply: FastifyReply) {
		let manifest = {
			...
			// 空文字列の場合右辺を使いたいため
			// eslint-disable-next-line @typescript-eslint/prefer-nullish-coalescing
			'theme_color': this.meta.themeColor || '#f5a9e0',
			'icons': [{
				// 空文字列の場合右辺を使いたいため
				// eslint-disable-next-line @typescript-eslint/prefer-nullish-coalescing
	...
	private async generateCommonPugData(meta: MiMeta) {
		return {
			instanceName: meta.name ?? 'Cafemis',
			icon: meta.iconUrl,
			appleTouchIcon: meta.app512IconUrl,
			themeColor: meta.themeColor,
			serverErrorImageUrl: meta.serverErrorImageUrl ?? '/client-assets/status/missingpage.webp',
			infoImageUrl: meta.infoImageUrl ?? '/client-assets/status/missingpage.webp',
	...
	public createServer(fastify: FastifyInstance, options: FastifyPluginOptions, done: (err?: Error) => void) {
		...
		fastify.get('/opensearch.xml', async (request, reply) => {
			const name = this.meta.name ?? 'Cafemis';
			let content = '';
