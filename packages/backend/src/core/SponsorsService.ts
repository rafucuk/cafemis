...
export class SponsorsService implements OnApplicationShutdown {
	...
	private async fetchSharkeySponsors() {
		try {
			const backers = await fetch('https://opencollective.com/cafemis/tiers/backer/all.json').then((response) => response.json());
			const sponsorsOC = await fetch('https://opencollective.com/cafemis/tiers/sponsor/all.json').then((response) => response.json());
	...
	public async sharkeySponsors(forceUpdate: boolean) {
		if (forceUpdate) this.cache.refresh('cafemis');
		return this.cache.fetch('cafemis');
	}
